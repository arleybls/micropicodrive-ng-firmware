...
#include "pico/flash.h"   //flash_safe_execute_core_init — both boards (trap T1)
...
//Microdrive control routine, this is the core0 main loop
void RunMDControl()
{
    //Core 1 writes flash via flash_safe_execute — display settings on BOTH
    //boards, plus the BLE/OTA modes (bond store, firmware slots) on the 2 W.
    //This core must opt in to the lockout handshake or every one of those
    //writes fails. All of them are gated on the cartridge being ejected, so
    //the XIP stall never hits a live QL transfer.
    //
    //Unconditional on purpose (study trap T1): this used to be wrapped in
    //#if UIEXT_OTA_ENABLED, i.e. the radio flag decided whether core 0 could
    //support core 1's settings writes. It is a no-op on both shipped products
    //— mainline builds with the flag set, Lite already called it — so this is
    //the removal of a wrong gate, not a new one. Bar clause 3 is preserved:
    //no NEW #if enters the timing-critical path.
